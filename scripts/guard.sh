...
case "$mode" in
    warn)
        if ! should_skip; then
            ...
            # into Tier 2.
            printf >&2 "\033[33m⚠  blueprint un-initialized — rebrand with \`uv run --locked press rebrand\` (see docs/POST_INIT.md).\033[0m\n"
        fi
        exit 0
        ;;
    block)
  ...
  Run one of:
      uv run --locked press rebrand --target . --config press-answers.toml
