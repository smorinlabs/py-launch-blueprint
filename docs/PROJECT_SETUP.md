# Project setup — decisions and configuration

This checklist configures the application after its identity has been pressed.
Review each retained capability before enabling publishing or optional services.
The source template is recorded in `press/press-receipt.toml` under `[press.from]`.
...
- [ ] **CodeQL code scanning** (advanced setup, `security-extended`) —
      *Default: on for public repositories; skipped for private repositories.*
...
- [ ] **Dependency review** (PR diff vuln/license check) — *Default: on.* File:
      `.github/workflows/dependency-review.yml`. The current workflow runs only on public repositories.
...
After the bootstrap has formatted the generated tree, run:
...
Use `bun install --frozen-lockfile` inside each additional Git worktree.
...
The shipped CodeQL and dependency-review workflows skip private repositories.
Keeping their files does not provide analysis there. The repository also has
Bandit, dependency auditing, gitleaks and TruffleHog checks; confirm the retained
checks run in the new repository. Configure any additional licensed services
explicitly.
